...
# 2. 先手动加载 依赖库 (libImageOperationLib.so)
ctypes.CDLL(os.path.join(cwd, "libImageOperationLib.so"), mode=ctypes.RTLD_GLOBAL)

# 3. 再加载 主库
_lib = ctypes.CDLL(os.path.join(cwd, "libkfbslide.so"))
